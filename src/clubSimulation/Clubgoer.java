...
public class Clubgoer extends Thread {
  ...
  private void checkPause() throws InterruptedException {
    // NEW - FIXED
	// Ensures that the threads sleep while the pause button is pressed, until it is pressed again
    while (pause.get()) {
      Thread.sleep(100);
    }
  }
  ...
  public void startSim() {
    // Left empty as the start button initialises the program
  }
  ...
  public void enterClub() throws InterruptedException {
    ...
    inRoom = true;
    System.out
        .println("Thread " + this.ID + " entered club at position: " + currentBlock.getX() + " " + currentBlock.getY());
  ...
  private void headToBar() throws InterruptedException {
    ...
    System.out.println(
        "Thread " + this.ID + " moved toward bar to position: " + currentBlock.getX() + " " + currentBlock.getY());
  ...
  private void headTowardsExit() throws InterruptedException {
    ...
    System.out
        .println("Thread " + this.ID + " moved to towards exit: " + currentBlock.getX() + " " + currentBlock.getY());
