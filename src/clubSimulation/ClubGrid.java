...
public class ClubGrid {
  ...
  public GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException {
    ...
    synchronized (entrance) {
      counter.personArrived(); 				// added to counter of people waiting outside
      while (counter.overCapacity()) {		// Condition if capacity is reached
        entrance.wait(); 					// ensures threads wait until there is space inside before entering
      }
    }
    // Given code - order rearranged
    entrance.get(myLocation.getID());
    counter.personEntered(); 				// add to counter of people inside
    myLocation.setLocation(entrance);		// Update location
  ...
  public GridBlock move(GridBlock currentBlock, int step_x, int step_y, PeopleLocation myLocation)
    ...
    synchronized (entrance) {
      ...
      boolean entryBlock = currentBlock.equals(entrance); 	// Checks if current block is entrance block
      if (entryBlock) {
        entrance.notifyAll(); 								// notify waiting threads that there is space inside
      }
